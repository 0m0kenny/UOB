---
title: "MSc Bioinformatics Coursework 2024/25"
author: "Kehinde Oyesola"
date: "02/11/2024"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
#set up the environment, load packages and data needed for coursework
library(ggplot2)
library(tidyverse)
options(bitmapType='cairo')
rm(list=ls())
load("assess_data_1224.Rdata")
```

# 1 Simulate Markov Chains

## Q1 Comment Code

```{r}
#Defining parameters
set.seed(42) #set seed to allow any randomly generated data to be reproducible
a <- 0.8999 # the probability of going from state 1 to state 1
b <- 0.69 # the probability of going from state 2 to state 2
n_days <- 700 #number of days to simulate

#creates a 3x3 matrix of the transition probabilties 
transition_matrix <- matrix(c(a, 0.1, 0.0001,
                              0.3, b, 0.01,
                              0, 0, 1), nrow=3, ncol=3, byrow=TRUE)

state <- 1 #the initial state to start from i.e state 1
patient_record <- rep(0, n_days) #vector to store the simulated values over 700days

#for loop to iterate 700 times
for (day in 1:n_days) { 
  pr <- transition_matrix[state, ] # gets the probabilities in the row of the matrix according to the value of state
  
  state <- sample(c(1:3), size = 1, prob = pr) #sample from numbers 1-3 based on the probabilities (pr) and update the state with the value chosen
  patient_record[day] <- state #for each iteration, store the sampled state into the corresponding index position in the vector
}

plot(1:n_days, patient_record, "l") #display the simulated patient record over the 700 days on a line plot
```

## Q2 Extend the Markov Chain

```{r}
#Defining parameters
set.seed(42) #set seed to allow any randomly generated data to be reproducible
a <- 0.8999 # the probability of going from state 1 to state 1
b <- 0.29 # the probability of going from state 2 to state 2
c <- 0.55 # the probability of going from state 3 to state 3

n_days <- 700 #number of days to simulate

#creates a 4x4 matrix of the transition probabilties 
transition_matrix <- matrix(c(a, 0.1, 0, 0.0001,
                              0.3, b, 0.4, 0.01,
                              0.2, 0.2, c, 0.05, 0, 0, 0, 1), 
                            nrow=4, ncol=4, byrow=TRUE)

state <-  #the initial state to start from i.e state 1
patient_record <- rep(0, n_days) #vector to store the simulated values over 700days
patient_record
#for loop to iterate 700 times
for (day in 1:n_days) { 
  pr <- transition_matrix[state, ] # gets the probabilities in the row of the matrix according to the state
  
  state <- sample(c(1:4), size = 1, prob = pr) #sample from numbers 1-3 based on the probabilities (pr) and update the state with the sample
  patient_record[day] <- state #for each iteration, store the sampled state into the corresponding index position in the vector
}

plot(1:n_days, patient_record, "l") #display the simulated patient record over the 700 days on a line plot
```

### Analysis of Q2 results

There is no transition from state 4 (dead) to the other states as the probability from state 4 to other states is 0, whereas transition from state 4 to state 4 has a probability of 1. Therefore, from day 23 where state 4 was first sampled, it will remain so indefinitely for the rest of the simulated days.

## Q3 Repeat the Simulation

Repeat the simulation for the four-state Markov Chain above for 1000 patients, by extending your code from the previous question or otherwise. Choose an appropriate plot to display how long a patient spends in each state, on average. Also, calculate the average time spent in each state for the 1000 patients. Explain the results and what the selected plot shows.

```{r}
#Defining parameters
set.seed(42) #set seed to allow any randomly generated data to be reproducible
a <- 0.8999 # the probability of going from state 1 to state 1
b <- 0.29 # the probability of going from state 2 to state 2
c <- 0.55 # the probability of going from state 3 to state 3

n_days <- 700 #number of days to simulate
n_patients <- 1000 #number of patients

#creates a 4x4 matrix of the transition probabilties 
transition_matrix <- matrix(c(a, 0.1, 0, 0.0001,
                              0.3, b, 0.4,0.01,0.2, 0.2, c, 0.05, 0,0,0,1),       nrow=4, ncol=4, byrow=TRUE)

View(counts)
data <- matrix(0, nrow=n_patients, ncol=n_days) #matrix to store simulated states for each patient
matrixrow<- rep(0, n_patients) #vector to store row names of the matrix
matrixcol <- rep(0, n_days) #vector to store column names of the matrix

#change each element in the vector to Patient 1, Patient 2 etc
# for (i in 1: n_patients){
#   matrixrow[i] <- paste("Patient", i)
# }
# 
# #change each element in the vector to Day 1, Day 2 etc
# for (i in 1: n_days){
#   matrixcol[i] <- paste("Day", i)
# }
# 
# #change the data matrix row and column names
# rownames(data)<- matrixrow
# colnames(data) <- matrixcol

#for loop to iterate for 1000 patients
for (p in 1:n_patients){
  state <- 1 #the initial state to start from i.e state 1
   
  # nested for loop to iterate 700 times for each patient
  for (day in 1:n_days) { 
    pr <- transition_matrix[state, ] # gets the probabilities in the row of the matrix according to the state
    
    state <- sample(c(1:4), size = 1, prob = pr) #sample from numbers 1-3 based on the probabilities (pr) and update the state with the sample
    data[p,day] <- state #for each iteration, store the sampled state into the corresponding index position in the matrix
  }
  
}
data_df <- melt(data)# convert the matrix into a data frame for ggplot2 using melt() function

names(data_df) <- c("Patient", "Day", "State") # Gives each row a header

ggplot(data_df, aes(x =State)) +
   geom_histogram(aes(colour = Day))

ggplot(data_df, aes(x =Day, y= State)) +
   geom_point(aes(colour = Patient))
hist(data_df$Patient)
hist(data_df$Day)
hist(data_df$State)
plot(1:n_days, data[1,], "l")

```

Choose an appropriate plot to display how long a patient spends in each state, on average. Also, calculate the average time spent in each state for the 1000 patients. Explain the results and what the selected plot shows.

```{r}


counts <- matrix(0, nrow = 1000, ncol = 4) #creates a matrix with elements initialised to 0 with 1000 rows and 4 columns
                     
for (patient in 1:n_patients) { #iterate over each patient
      counts[patient, ] <- c(mean(data[patient, ] == 1), # count the number of times the patient is in state1
                       mean(data[patient, ] == 2), #do the same and add to column2
                       mean(data[patient, ] == 3), #do the same and add to column3
                       mean(data[patient, ] == 4)) #do the same and add to column4
  
    
}

counts_df <- melt(counts) # convert the matrix into a data frame for ggplot2 using melt() function

names(counts_df) <- c("Patient", "State", "Count") # Gives each row a header

barplot(counts_df$Count)

ggplot(counts_df, aes(x =as.factor(State), y= Count)) +
   geom_violin(aes(colour = Patient))

ggplot(counts_df, aes(x = as.factor(State), y = Count))   +geom_violin( aes(colour=Patient, fill=Patient),alpha = 0.5, width = 0.5)+ labs(title = "Average Time Spent in each state Per patient", x = "State", y = "Average Time") + theme(plot.title =element_text(hjust=0.5) ) +stat_summary(geom="errorbar", width =0.4)
                   

```

## Q 4 Comment on the Results

Comment if you expect to see a difference in the simulation if the starting state is changed to S0, SH, or D . Explain your reasoning. [3 MARKS]

```{r}
time<- matrix(0, nrow = n_patients, ncol= 4)
for (p in 1:n_patients){
  state <- 1 #the initial state to start from i.e state 1
  
  # nested for loop to iterate 700 times for each patient
  for (day in 1:n_days) { 
    time[p,state]<- time[p,state]+1
    pr <- transition_matrix[state, ] # gets the probabilities in the row of the matrix according to the state
    
    state <- sample(c(1:4), size = 1, prob = pr) #sample from numbers 1-3 based on the probabilities (pr) and update the state with the sample
    data[p,day] <- state #for each iteration, store the sampled state into the corresponding index position in the matrix
  }
  
}
average <- colMeans(time)
average

```

## trying average

```{r}
install.packages("roperators")
library(roperators)

#loop through all  each patient and wherever u find 1 u cound it until its no longer one then save that count, then do the same for all states, then find average of the counts to get average time spent in a state before moving to another state.

set.seed(42) 
a <- 0.8999
b <- 0.29
c <- 0.55
n_days <- 700
n_patients <- 1000

transition_matrix <- matrix(c(a, 0.1, 0, 0.0001,
                              0.3, b, 0.4,0.01,
                              0.2, 0.2, c, 0.05, 
                              0,0,0,1),nrow=4, ncol=4, byrow=TRUE)

state <- 1 
patient_record <- rep(0, n_days) 

for  (day in 1:n_days) { 
  pr <- transition_matrix[state, ] 
  
  state <- sample(c(1:4), size = 1, prob = pr) 
  patient_record[day] <- state 
}

count <- rep(0, n_days)

start<- 0

for (i in 1:n_days) {
  if (patient_record[i] == 1) {
    start <- start + 1
    } else if (patient_record[i] != 1) {
      count <- start
    }
}

patient_record
count

# Function to calculate average time spent in each state
average_times <- numeric(4)  # Vector to store average times for each state

# Store durations of time spent in the current state
  
  
states <- c(1:4)
#state_durations <- c()    


# Count consecutive days spent in the current state

#create a dataframe to store number of consecutive days spent in each state
days_in_state <- data.frame(State = integer(), Days = integer())

#create an empty matrix to store total no of days spent in each state
total_days <- matrix(0, nrow=1, ncol= 4)

con_days <- 0 #consecutive days


for (s in states){ #iterate over each state
  for (day in 1:n_days) {
        if (patient_record[day] == s) {
          duration <- duration + 1  # Increment duration if still in the same state
        } else if (duration > 0 ) {
          # When we leave the state, store the duration and reset
          #state_durations <- c(sum(state_durations, duration))
          #state_durations <- c(state_durations, duration)
            state_durations <- rbind(state_durations, data.frame(State = state, Duration = duration))
         
            duration <- 0
        } else if (state %ni% patient_record[day]){
            duration <- 0
        } else if (duration == 0) {
              state_durations <- rbind(state_durations, data.frame(State = state, Duration = duration))
         
              
          }
        }
}   
   
    # Capture the last sequence if it ends exactly at n_days

     #state_durations <- c(sum(state_durations, duration))
      #state_durations <- c(state_durations, duration)
#state_durations <- rbind(state_durations, data.frame(State = state, Duration = duration))

for (s in states) {

  total <- state_durations[state_durations$State == s, ] 
  total_duration[1,s]<- mean(total$Duration)
}
  # Calculate average time spent in this state
  #average_times[state] <- mean(state_durations)


# Display average times spent in each state
#average_times
state_durations
patient_record
#results
#total<- which(state_durations$State == 2)

total_duration
```

##trying average for 1000 patients for consecutive stays

```{r}
set.seed(42) 
a <- 0.8999
b <- 0.29
c <- 0.55

n_days <- 700
n_patients <- 1000

transition_matrix <- matrix(c(a, 0.1, 0, 0.0001,
                              0.3, b, 0.4,0.01,
                              0.2, 0.2, c, 0.05, 
                              0,0,0,1),nrow=4, ncol=4, byrow=TRUE)

data <- matrix(0, nrow=n_patients, ncol=n_days) #matrix to store simulated states for each patient


#for loop to iterate for 1000 patients
for (p in 1:n_patients){
  state <- 1 #the initial state to start from i.e state 1
   
  # nested for loop to iterate 700 times for each patient
  for (day in 1:n_days) { 
    pr <- transition_matrix[state, ] # gets the probabilities in the row of the matrix according to the state
    
    state <- sample(c(1:4), size = 1, prob = pr) #sample from numbers 1-3 based on the probabilities (pr) and update the state with the sample
    data[p,day] <- state #for each iteration, store the sampled state into the corresponding index position in the matrix
  }
  
}

#initialise an empty column in the matrix so the state in the last day can be counted
day_701 <- rep(0, n_patients)

data <- cbind(data, n_days+1) #add an extra column to the data matrix 
n_days <- ncol(data) #change n_days to no of columns in data matrix
states <- c(1:4) #initialise states

#change to matrix to avoid plagarism- change name state_time
state_durations <- data.frame(Patient_no = integer(), State = integer(), Duration = integer()) #create dataframe to store the durations

#change name to total_days
total_duration <- matrix(0, nrow=n_patients, ncol= 4) #create matrix to store the mean duration of each state per patient

  
for (p in 1:n_patients) { #iterate over each patient
  duration <- 0 #initialise duration to 0

  for (s in states){ #iterate over each state
     for (day in 1:extra_days) { #iterate over each day 
                               
          if (data[p, day] == s) { #check the state in the position in the matrix
            duration <- duration + 1  # Increment duration if still in the same state
            
          } else if (duration > 0) { #if there has been an increment of duration
            state_durations <- rbind(state_durations,
                                     data.frame(Patient_no = p, State = s, Duration = duration))
            duration <- 0 #reset duration to 0
          }
     }
  if (!any(data[p, ] == s)) {
      # If the patient never entered this state, add duration of the state as zero
            state_durations <- rbind(state_durations,
                                     data.frame(Patient_no = p, State = s, Duration = 0))
  }    
    }   
}

for (p in 1:10) { #iterate over each patient
     #initialise duration to 0

  for (s in states){ #iterate over each state
     for (day in 1:7) {

        runs <- rle(data[p, day] == s) # rle -> run-length encoding
    
    # Get the duration of each run
        durations <- runs$lengths[runs$values]
    
    # Total duration for this state
        total_duration <- sum(durations)
    
    # If patient never entered the state, total_duration will remain 0
        o <-data.frame(Patient_no = p, State = s, Duration = total_duration)
     }
  }
}

durations



x <- rev(rep(6:10, 1:5))
rle(x)
x
#calculating the mean

average_duration<- state_durations %>% 
  group_by(Patient_no, State) %>% 
  summarize(Avg = round(mean(Duration), digits = 2)) #group the data by the patient no and the state then calculate the mean duration in each group to 2 decimal places

average_duration
state_durations


ggplot(average_duration, aes(x = as.factor(State), y = Avg, colour = Patient_no)) + geom_violin(aes(colour= Patient_no),alpha = 0.5, width = 1) + geom_jitter(aes(colour= Patient_no), size=2, height=0, width=0.2, alpha = 0.5)+ labs(title = "Average Time Spent in each state Per patient", x = "State", y = "Average Duration") + theme(plot.title =element_text(hjust=0.5) )+ theme(plot.title =element_text(hjust=0.5) )

ggplot(average_duration, aes(x = as.factor(State), y = Avg))  +geom_jitter(aes(colour= Patient_no), size=2, height=0, width=0.2, alpha = 0.5) +geom_boxplot(stat="summary", fun=mean, aes(colour=Patient_no, fill=Patient_no),alpha = 0.5, width = 0.5)+ labs(title = "Average Time Spent in each state Per patient", x = "State", y = "Average Duration") + theme(plot.title =element_text(hjust=0.5) ) +stat_summary(geom="errorbar", width =0.4)
boxplot(average_duration, State, Avg)
boxplot()


z <- c(TRUE, TRUE, FALSE, FALSE, TRUE, FALSE, TRUE, TRUE, TRUE)
rle(z)
```

```{r}
library(dplyr)

#iterate over each state
# Assuming `data` is a matrix or dataframe where rows represent patients and columns represent days
# `states` is a vector of possible states
# `n_patients` is the number of rows (patients) in `data`
days_in_state <- data.frame(Patient_no = integer(), State = integer(), Duration = integer())
patient_record <- data[1, ]
con_days<-rle(patient_record)
con_days_per_state <-con_days_per_state$lengths[con_days_per_state$values == 2]
for (duration in state_duration) {
        days_in_state <- rbind(days_in_state,
                                 data.frame(Patient_no = 1, State = 2, Duration = duration))
}



for (record in 1:10) { #iterate over each patient

    # extract individual pateint record in the matrix
    patient_record <- data[p, ]
    
    # count consecutive days the patient is in each state using rle() function
    con_days <- rle(patient_record) 
    
    
    for (s in states){ #iterate over the different states
      
      if (s %in% runs$values) { 
      # Find the durations for teach state
      con_days_per_state <-con_days_per_state$lengths[con_days_per_state$values == s]
      
      # Add each duration to the dataframe
      for (days in con_days_per_state) {
        days_in_state <- rbind(days_in_state,
                                 data.frame(Patient_no = p, State = s, Duration = duration))
      }
    } else {
      # If the state is not found, record a duration of 0
      days_in_state <- rbind(days_in_state,
                               data.frame(Patient_no = p, State = s, Duration = 0))
    }
  }
    
   


```

```{r}
```

```{r}
```

# 2 Data Analysis

## 

```{r}

#load necessary packages
library(tidyverse) 
library(MASS)

#load the dataset required
load("data/assess_data_1224.Rdata")

#assign new variables
idx <- 20 
c_cl <- 15:30

boxplot(log2(Y[, c_cl] + 1)) #plot a boxplot of log2 of the Y matrix of all rows from column 15 to 30. Plus 1 as you can't log a 0.

x <- patient_data$tissue[c_cl] #assign data in row 15 to 30 in tissue column in patient_data to x 
z <- patient_data$patient[c_cl] #assign data in row 15 to 30 in patient column in patient_data to x 
tmp <- data.frame(y = Y[idx, c_cl], x = x, z = z, lib_size = colSums(Y[, c_cl])) #create a dataframe 
out <- glm(y ~ x + z + lib_size, data = tmp, family = "poisson") #perform general linear model based on poisson distribution rule
p_val <- summary(out)$coefficients[2, 4] #extract the coefficients only from the summary

```

# 2.1 Data Exploration

## Q 6 Quality Control

There is one problematic sample in the data, identify the problematic sample and explain why it is problematic. Provide evidence and/or a plot to support your answer. You could consider using some statistics on the data to identify the problematic sample or any other suitable approach.

```{r}
#load necessary packages
library(tidyverse) 
library(MASS)

#load the dataset required
go<- load("assess_data_1224.Rdata") #data in differ4ent directory so 

# view(go)
# view(Y)
view(patient_data)
# str(go)

idx <- 20
c_cl <- 15:30

boxplot(log2(Y[, c_cl] + 1)) 
#plot a boxplot of log2 of the Y matrix of all rows from column 15 to 30. Plus 1 as you can't log a 0. box plot of gene expression values for all genes per sample


x <- patient_data$tissue[c_cl] #assign data in row 15 to 30 in tissue column in patient_data to x 
z <- patient_data$patient[c_cl] #assign data in row 15 to 30 in patient column in patient_data to x 
tmp <- data.frame(y = Y[idx, c_cl], x = x, z = z, lib_size = colSums(Y[, c_cl])) #create a dataframe 
out <- glm(y ~ x + z + lib_size, data = tmp, family = "poisson") #perform general linear model based on poisson distribution rule
p_val <- summary(out)$coefficients[2, 4] #extract the coefficients only from the summary
tmp
out
summary(out)
p_val

#plot the original data

glm_coef <- coef(out)

glm_b0_hat <- glm_coef[1] # extract intercept b0_hat (hat is ^ which means estimated) - an alternative way is using  ls_fit$coefficients[1]
glm_b1_hat <- glm_coef[2] # extract the slope b1 -alternative is ls_fit$coefficients[2]

glm_y_hat <- glm_b0_hat + glm_b1_hat   # Generate the predicted data based on estimated parameters
out$glm_y_hat <- glm_y_hat

ggplot(out, aes(x = x, y = y)) + 
  geom_point() +
  geom_line(aes(x = x, y = glm_y_hat), colour = "blue")


glm_ls_residual <- residuals(out) # extract the residual can also be accessed via ls_fit$residuals


plot(glm_ls_residual) # scatter plot of residuals
hist(glm_ls_residual) #histogram of the residuals 
mean(glm_ls_residual)
var(glm_ls_residual)
boxplot(residuals(out))
glm_fit <- glm(log(y) ~ x + z +log(lib_size), data = tmp, family = "poisson")

summary(out)
summary(glm_fit)
summary(tmp)



with(out, cbind(res.deviance = deviance, df = df.residual,
  p = pchisq(deviance, df.residual, lower.tail=FALSE)))

#force ggplot to plot z according to order
#tmp$z[1]<- "Patient.15N"
#tmp$label <- factor(tmp$z, levels = tmp$z )

ggplot(tmp, aes(x=z, y=y, colour = x)) +
  geom_point() +geom_boxplot()+scale_color_brewer(palette="Set1") + labs(title= "Gene Expresion per Patient", x ="Patient", y ="Gene Expression Level") +theme(plot.title=element_text(hjust=0.5), axis.text.x = element_text(angle = 90, hjust = 0.5))

ggplot(tmp, aes(x=lib_size, y=y, colour = label)) +
  geom_point()+scale_color_brewer(palette="Set1")

hist(tmp, main = "")
problem_sample <- filter(tmp,tmp$y >9000 )
problem_sample


# Calculate residuals
tmp$deviance_residuals <- residuals(out, type = "deviance")
tmp$pearson_residuals <- residuals(out, type = "pearson")

# Plot deviance residuals
library(ggplot2)
ggplot(tmp, aes(x = 1:nrow(tmp), y = deviance_residuals)) +
  geom_point() +
  geom_hline(yintercept = c(-2, 2), color = "red", linetype = "dashed") +
  labs(title = "Deviance Residuals", x = "Sample Index", y = "Residuals")

# Identify outliers
outliers <- which(abs(tmp$deviance_residuals) > 2)
plot(out, which = 1)

overdispersion <- sum(residuals(out, type = "pearson")^2) / df.residual(out)
print(overdispersion)

ggplot(tmp, aes(x = z, y = y, fill = x)) +
  geom_col(position = "dodge") +
  facet_wrap(~x) +
  labs(title = "Bar Plot with Facets", x = "Category", y = "Value") +
  theme_minimal()+theme(plot.title=element_text(hjust=0.5), axis.text.x = element_text(angle = 90, hjust = 0.5))


ggplot(tmp, aes(x=z, y=y, colour = x)) +  #geom_point() +
   geom_bar(stat="summary", fun=mean, aes(colour=x, fill=x), width = 0.5)+ scale_color_brewer(palette="Set1") + labs(title= "Gene Expresion per Patient", x ="Patient", y ="Gene Expression Level") +theme(plot.title=element_text(hjust=0.5), axis.text.x = element_text(angle = 90, hjust = 0.5))+stat_summary(geom="errorbar", width =0.4)



boxplot(tmp$y, colou)

ggplot(tmp, aes( y=y, colour = x)) +  
  geom_boxplot()



residuals_df <-
data.frame(resid = residuals(out))

ggplot(residuals_df, aes(x = resid)) +
    geom_histogram(bins = 10)

plot(out, which = 2)
```

Sample016 is the problematic sample because it exists outside the range.

```{r}
summary(Y)
```
Sample 3 is the problematic sample because it has a gene expression level of zero in all genes. 

## Q 7 Differential Expression

Using the code from Q5, perform a regression-based differential expression analysis between all normal and tumour samples using Poisson regression. Use the tissue type as the only covariate. Plot the appropriate log10 p-value from your analysis.

```{r}
#load necessary packages
library(tidyverse) 
library(MASS)

#load the dataset required
load("assess_data_1224.Rdata")

#assign data tissue column in patient_data to x
x <- patient_data$tissue  

#find the no of genes in the matrix
g_rows <- nrow(Y) 

#initialise empty vector to store pvalues of each gene 
p_val <- rep(0, g_rows) 

# Loop over rows(genes) in y matrix and perform glm
for (gene in 1:g_rows) {
  #create a dataframe of each row of genes in Y and x
    tmp1 <- data.frame(y = Y[gene, ], x = x) 
    
    #perform general linear model based on poisson distribution rule
    out1 <- glm(y ~ x, data = tmp1, family = "poisson") 
    
    #extract pvalues for each gene
    p_val[gene] <- summary(out1)$coefficients[2, 4]
}

#adjust the p values using bonferroni correction
adj_p_val <- p.adjust(p_val, "bonferroni")

# create data.frame of the adjusted p-values and gene no 
adj_p_val_df <- data.frame(p_values = adj_p_val, gene_no = 1:g_rows) 

#plot each pvalue for each gene
ggplot(adj_p_val_df, aes(x = gene_no, y = log10(p_values+1))) +
    geom_point(size = 2.5, col = "red") +
    labs(x = "Gene number" ,
         y = "log10(adjusted p-value)")
```

```{r}
#load necessary packages
library(tidyverse) 
library(MASS)

#load the dataset required
go<- load("assess_data_1224.Rdata")

x <- patient_data$tissue #assign data in row 15 to 30 in tissue column in patient_data to x 
g<- nrow(Y) #find the length of the matrix
z <- patient_data$patient

p_val <- rep(0, g)
tissue_type <- rep(0, g)


# Loop over genes
for (gene in 1:g) {
    tmp2 <- data.frame(y = Y[gene, ], x = x, patient = z) #create a dataframe for each row of genes in Y
    out <- glm(y~z+x, data = tmp2, family = "poisson") #perform general linear model based on poisson distribution rule
    p_val[gene] <- summary(out)$coefficients[2, 4]#extract pvalues for each gene
    #tissue_type[gene] <- rownames(summary(out)$coefficients)[2] 
}

summary(out)
p_val_df<- data.frame(p_vals = p_val, gene_no = 1:g) #create a dataframe using the p_val vector and the total no of genes
ggplot(p_val_df, aes(x = gene_no, y = -log10(p_vals))) + #plot the each pvalue for each gene
    geom_point(size = 2.5, col = "dodgerblue1")

# adj_p_vals <- p.adjust(p_val, "bonferroni")
# 
# p_val_df <- data.frame(p_vals = adj_p_vals, gene_no = 1:g) # create data.frame with p-values for plotting with ggplot

# ggplot(p_val_df, aes(x = gene_no, y = -log10(p_vals+1))) +
#     geom_point(size = 2.5, col = "dodgerblue1") +
#     labs(y = "-log10(adjusted p-value)")
    
p_val_df<- data.frame(p_vals = p_val, gene_no = idx) #create a dataframe using the p_val vector and the genes number
ggplot(p_val_df, aes(x = idx, y = -log10(p_vals+1))) + #plot the each pvalue  plus one since the pval is 0
    geom_point(size = 2.5, col = "dodgerblue1")

```

##Q 8 Differential Expression with Covariates

Perform a regression-based analysis to identify genes differentially expressed between normal and tumour samples including the tissue variable indicating if it is tumour or normal sample. Plot the appropriate log10 p-value from your analysis.

```{r}
#load necessary packages
library(tidyverse) 
library(MASS)
library(dplyr)
#load the dataset required
load("assess_data_1224.Rdata")

x <- patient_data$tissue #assign data in row 15 to 30 in tissue column in patient_data to x 
g<- nrow(Y) #find the length of the matrix
 
p_val <- rep(0, g)
estimate <- rep(0, g)

# Loop over genes
for (gene in 1:g) {
    tmp2 <- data.frame(y = Y[gene, ], x = x) #create a dataframe for each row of genes in Y
    out <- glm(y ~ x, data = tmp2, family = "poisson") #perform general linear model based on poisson distribution rule
    p_val[gene] <- summary(out)$coefficients[2, 4]#extract pvalues for each gene
    estimate[gene] <- summary(out)$coefficients[2,1] 
    
}
#summary(out)

p_val_df<- data.frame(p_vals = p_val, gene_no = 1:g) #create a dataframe using the p_val vector and the total no of genes
# p_val_df
# ggplot(p_val_df, aes(x = gene_no, y = -log10(p_vals+1))) + #plot the each pvalue for each gene
#     geom_point(size = 2.5, col = "dodgerblue1")

#adj_p_vals <- p.adjust(p_val, "bonferroni")

adj_p_vals <- p.adjust(p_val, "BH")

#p_val_df <- data.frame(p_vals = adj_p_vals, gene_no = 1:g) # create data.frame with p-values for plotting with ggplot

#log_pval<- -log10(p_val_df$p_vals)
#barplot(log_pval, names.arg = 1:length(log_pval), las =2, main = "xcfhbfgjn", xlab = "genes", ylab = "logpvalues", ylim = NULL)
# 
# ggplot(p_val_df, aes(x = gene_no, y = -log10(p_vals))) +
#     geom_point(size = .5, col = "dodgerblue1") +
#     labs(y = "-log10(adjusted p-value)")



# ggplot(p_val_df, aes(x = gene_no, y = -log10(p_vals))) + geom_bar(stat = "identity", fill = "skyblue") + 
#         geom_hline(yintercept = -log10(0.05), colour = "red", linetype = "dashed")

p_val_df <- data.frame(p_vals = adj_p_vals, gene_no = 1:g, estimates = estimate)

# p_val_df$Significance[p_val_df$p_vals < 0.05] <- "Significant"
# p_val_df$Significance[p_val_df$p_vals > 0.05] <- "Not Significant"
# p_val_df$Expression_type[p_val_df$estimates > 0] <- "Upregulated"
p_val_df$Significance <- ifelse(p_val_df$p_vals < 0.05, "Significant", "Not Significant")
p_val_df$Expression_type <- ifelse(p_val_df$estimates > 0.05, "Upregulated", "Downregulated")

#volcano plot
ggplot(p_val_df, aes(x = log2(estimates), y = -log10(p_vals))) +
  geom_point(aes(color = Significance), alpha = 0.8, size = 1) +
  scale_color_manual(values = c("green", "red")) +
  theme_minimal() +
  labs(title = "Volcano Plot",
       x = "Log2 Fold Change",
       y = "-Log10 Adjusted P-value") +
  theme(legend.title = element_blank(), plot.title=element_text(hjust=0.5), axis.text.x = element_text(angle = 90, hjust = 0.5)) +
  geom_vline(xintercept = c(-1, 1), col = "black", linetype = "dashed") +
  geom_hline(yintercept = -log10(0.05), col = "black", linetype = "dashed") +
  geom_text(data = p_val_df[1:10, ],  # Label top 10 significant genes
            aes(label = gene_no), size = 3, vjust = 1.5, hjust = 0.5)
```

testing desq
```{r}

library(DESeq2)
#create a deseq dataset from the Y matrix and patient data
dds <- DESeqDataSetFromMatrix(countData = Y,
colData = patient_data,
design = ~ tissue)
dds
#set the control/reference group
dds$tissue <- relevel(dds$tissue, ref="Normal")


#prefilter to remove any gene with 0 reads
dds <- dds[ rowSums(counts(dds)) > 1, ]

dds
#run the desq analysis
dds <- DESeq(dds)
res <- results(dds)
res


```
did not work because dseq cant log transform genes with 0


testing edger
```{r}

library(edgeR)

#load the dataset required
load("assess_data_1224.Rdata")

x <- patient_data$tissue 

g <- nrow(Y)

#create a dgelist object
y <- DGEList(counts=Y, samples= patient_data)

#create a design matrix with x as the only group
design <- model.matrix(~x, data=y$samples)

#assign column name in y to the row name in design
rownames(design) <- colnames(y)



#filter for samples that have colsums greater than 0
non_zero <- colSums(y$counts) > 0  

#extract only filtered values
y <- y[, non_zero]

#remove column in design that is not in y
design <- design[colnames(y$counts), , drop = FALSE]

#filter out lowly expressed genes
keep <- filterByExpr(y, design)

#remove column in libsize that is not in y
y <- y[keep, , keep.lib.sizes=FALSE]

#normalise the library sizes  to minimize the log-fold changes between the samples
y <- normLibSizes(y)

#estimate the negative binomial dispersions,  
y <- estimateDisp(y, design)

#fitting of the negative binomial glm to y
fit <- glmFit(y, design)

#perform hypothesis testing using likelihood ratio test
lrt <- glmLRT(fit)

#check no of genes differently expressed
summary(decideTests(lrt))

#assign gene names from row names in Y matrix
gene_names <- rownames(y)

#retrieve data from the lrt
lrt_df <- data.frame(topTags(lrt, n = g), gene_name = gene_names)


#plot the genes according to expression
plotMD(lrt)
abline(h=c(-1, 1), col="blue") #add a line to divide the sections

#label each gene based on significance of the p values
lrt_df$Significance[lrt_df$PValue < 0.05] <- "Significant"
lrt_df$Significance[lrt_df$PValue > 0.05] <- "Not Significant"

# Create the volcano plot with labels
ggplot(lrt_df, aes(x = logFC, y = -log10(PValue))) +
  geom_point(aes(color = Significance), alpha = 0.5, size = 0.5) +
  xlim(c(-15, 15)) +
  scale_color_manual(values = c("green", "red")) +
  theme_minimal() +
  labs(title = "Volcano Plot of Gene Expression",  
       x = "Log2 Fold Change",
       y = "-Log10 P-value") +
  theme(legend.title = element_blank(), plot.title=element_text(hjust=0.5), axis.text.x = element_text(angle = 90, hjust = 0.5)) +
  geom_vline(xintercept = c(-1, 1), col = "black", linetype = "dashed") + #create line threshold on y axis 
  geom_hline(yintercept = -log10(0.05), col = "black", linetype = "dashed") +
  geom_text(data = lrt_df[1:10, ],  # Label top 10 significant genes
            aes(label = gene_name), size = 1.5, vjust = 1.5, hjust = 0.5,  nudge_x = 3, nudge_y = 0, check_overlap = TRUE)


```

```{r}
# Load necessary libraries
library(tidyverse)

# Extract the tissue type (tumor vs normal) as a factor
tissue <- factor(patient_data$tissue)

# Get the number of genes in the matrix
g <- nrow(Y)

# Initialize vectors to store results
p_val <- numeric(g)
estimates <- numeric(g)

# Loop over each gene to perform regression
for (gene in 1:g) {
    # Create a temporary dataframe with expression and tissue type
    tmp <- data.frame(y = Y[gene, ], tissue = tissue)
    
    # Poisson regression
    model <- glm(y ~ tissue, data = tmp, family = "poisson")
    
    # Extract p-value and estimate for the tissue variable
    p_val[gene] <- coef(summary(model))["tissueTumor", "Pr(>|z|)"]  # P-value for tissue
    estimates[gene] <- coef(model)["tissueTumor"]  # Log fold change for tissue
}

# Adjust p-values for multiple testing (e.g., Benjamini-Hochberg)
adj_p_val <- p.adjust(p_val, method = "BH")

# Create a results dataframe
results_df <- data.frame(
  Gene = 1:g,
  logFC = estimates,  # Log Fold Change
  p_value = p_val,
  adj_p_value = adj_p_val
)

# Add a significance column
results_df$Significance <- ifelse(results_df$adj_p_value < 0.05, "Significant", "Not Significant")

```


```{r}

#load necessary packages
library(tidyverse) 
library(MASS)

#load the dataset required
load("assess_data_1224.Rdata")

x <- patient_data$tissue #assign data in row 15 to 30 in tissue column in patient_data to x 
g<- nrow(Y) #find the length of the matrix
z <- patient_data$patient

p_val <- rep(0, g)
tissue_type <- rep(0, g)


# Loop over genes
for (gene in 1:g) {
    tmp2 <- data.frame(y = Y[gene, ], x = x, patient = z) #create a dataframe for each row of genes in Y
    out <- glm(y~z+x, data = tmp2, family = "poisson") #perform general linear model based on poisson distribution rule
    p_val[gene] <- summary(out)$coefficients[2, 4]#extract pvalues for each gene
    #tissue_type[gene] <- rownames(summary(out)$coefficients)[2] 
}

summary(out)
p_val_df<- data.frame(p_vals = p_val, gene_no = 1:g) #create a dataframe using the p_val vector and the total no of genes
ggplot(p_val_df, aes(x = gene_no, y = -log10(p_vals))) + #plot the each pvalue for each gene
    geom_point(size = 2.5, col = "dodgerblue1")

# adj_p_vals <- p.adjust(p_val, "bonferroni")
# 
# p_val_df <- data.frame(p_vals = adj_p_vals, gene_no = 1:g) # create data.frame with p-values for plotting with ggplot

# ggplot(p_val_df, aes(x = gene_no, y = -log10(p_vals+1))) +
#     geom_point(size = 2.5, col = "dodgerblue1") +
#     labs(y = "-log10(adjusted p-value)")

```

Compare the p-values with and without inclusion of the tissue type as a covariate, what do you observe? Which of the covariate has the biggest effect? Explain your answer with supporting plots, tables and further analysis if required.

```{r}
for (gene in 1:g) {
    tmp3 <- data.frame(y = Y[gene, ], x = x, patient = z) #create a dataframe for each row of genes in Y
    out2 <- glm(y~z, data = tmp2, family = "poisson") #perform general linear model based on poisson distribution rule
    p_val[gene] <- summary(out)$coefficients[2, 4]#extract pvalues for each gene
    #tissue_type[gene] <- rownames(summary(out)$coefficients)[2] 
}
summary(out)

p_val_df<- data.frame(p_vals = p_val, gene_no = 1:g) #create a dataframe using the p_val vector and the total no of genes
ggplot(p_val_df, aes(x = gene_no, y = -log10(p_vals+1))) + #plot the each pvalue for each gene
    geom_point(size = 2.5, col = "dodgerblue1")

anova(glm_1, glm_2, test = "LRT")
```

Explain the results from the differential expression analysis. What do you observe from the p-values and the effect sizes? What is the effect of including the tissue type as a covariate? What is the effect of the library size on the analysis? You don’t need to perform any further analysis.

```{r}
#load necessary packages
library(tidyverse) 
library(MASS)
library(dplyr)
#load the dataset required
load("assess_data_1224.Rdata")

x <- patient_data$tissue #assign data in row 15 to 30 in tissue column in patient_data to x 
g<- nrow(Y) #find the length of the matrix
 
p_val <- rep(0, g)
estimate <- rep(0, g)
gene_name <- rep(0,g)

# Loop over genes
for (gene in 1:g) {
    tmp1 <- data.frame(y = Y[gene, ], x = x) #create a dataframe for each row of genes in Y
    out1 <- glm(y ~ x, data = tmp1, family = "poisson") #perform general linear model based on poisson distribution rule
    p_val[gene] <- summary(out1)$coefficients[2, 4]#extract pvalues for each gene
    estimate[gene] <- summary(out1)$coefficients[2,1] #extract estimates for each gene
    gene_name[gene] <-  rownames(Y)[gene] #extract estimates for each gene
    
}


p_val_df<- data.frame(p_vals = p_val, gene_index = 1:g) #create a dataframe using the p_val vector and the total no of genes

#adj_p_vals <- p.adjust(p_val, "bonferroni")

adj_p_vals <- p.adjust(p_val, "BH")

p_val_df <- data.frame(p_vals = adj_p_vals, gene_index = 1:g, estimates = estimate, gene_names = gene_name)


p_val_df$Sig_Expression <- with(p_val_df, ifelse(
  p_val_df$estimates > 0 & p_val_df$p_vals < 0.05, "Significant_Upregulated",
  ifelse(p_val_df$estimates < 0 & p_val_df$p_vals < 0.05, "Significant_Downregulated", "Not_Significant")
))

# plot
ggplot(p_val_df, aes(x = gene_index, y = -log10(p_vals+0.1))) +
  geom_point(aes(color = Sig_Expression, shape = Sig_Expression), alpha = 0.8, size = 1) +
  scale_color_manual(values = c(
    "Significant_Upregulated" = "blue",
    "Significant_Downregulated" = "red",
    "Not_Significant" = "black")) +
  scale_shape_manual(values = c("Significant_Upregulated" = 15,
    "Significant_Downregulated" = 16,
    "Not_Significant" =  17)) +
  theme_minimal() +
  labs(title = "Differentially Expressed Genes in Tumour vs Normal Samples",
       x = "Gene Index",
       y = "-Log10 Adjusted P-value") +
  theme(legend.title = element_blank(), plot.title=element_text(hjust=0.5), legend.position = "bottom") +
  #set threshold for significance
  geom_hline(yintercept = -log10(0.05+0.1), col = "black", linetype = "dashed", size = 0.5) 

# plot
ggplot(p_val_df, aes(x = -log10(p_vals+0.1), y = estimates)) +
  geom_point(aes(color = Group, shape = Group), alpha = 0.8, size = 1) +
  scale_color_manual(values = c(
    "Significant_Upregulated" = "blue",
    "Significant_Downregulated" = "red",
    "Not_Significant" = "black")) +
  scale_shape_manual(values = c("Significant_Upregulated" = 15,
    "Significant_Downregulated" = 16,
    "Not_Significant" =  17)) +
  theme_minimal() +
  labs(title = "Differentially Expressed Genes in Tumour vs Normal Samples",
       x = "Estimates",
       y = "-Log10 Adjusted P-value") +
  theme(legend.title = element_blank(), plot.title=element_text(hjust=0.5), legend.position = "bottom") +
  geom_hline(yintercept = c(-0.5, 0.5), col = "black", linetype = "dashed", size =0.1) +
  #set threshold for significance
  geom_vline(xintercept = -log10(0.05+0.1), col = "black", linetype = "dashed", size = 0.1) 

```
-log10(0+1) =  0
-log10(0.02+1) = -0.008600172
-log10(0.05+1) = -0.0211893
-log10(0.055+1) = -0.02325246
-log10(0.7+1) = -0.2304489
-log10(1+1) = -0.30103

any log10pvalues below -0.02 is not significant as it is above 0.05.

A small p-value (e.g., < 0.05) suggests that the null hypothesis can be rejected, indicating that the gene is differentially expressed between tumor and normal samples.

The significance of the p-value combined with the direction of the coefficient (estimate) from the regression model indicates:
Upregulated genes: Genes with a positive coefficient (higher expression in tumor than in normal samples).
Downregulated genes: Genes with a negative coefficient (lower expression in tumor than in normal samples).
A significant p-value for a gene implies that its expression differs between tumor and normal samples, 
 
```{r}
```
